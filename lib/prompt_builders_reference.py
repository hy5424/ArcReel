...
【禁止标签】：禁止[事项1]。禁止[事项2]。禁止图标与emoji。结尾保持静止不漂移。
```

## 核心规则

**格式**：每 shot 必填【站位】+【音色】+【禁止标签】（负面款以"禁止"开头，结尾含防漂移）。时间块秒数和=shot.duration。场景@仅切换时写。**台词必带情绪描述**——@角色名（焦急万分/声如洪钟/手指攥紧衣角）："台词"。**所有 @ 引用后必须加空格**（正确：`@林晏 端托盘`，错误：`@林晏、@沈栀` 和 `@林晏端托盘`）。**禁止画面中出现图标、emoji、文字、字幕**。references 按 @引用顺序。
